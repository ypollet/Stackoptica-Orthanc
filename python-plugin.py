# Stackoptica - Viewer of stacked images - Orthanc Plugin
...
def compute_landmark(output, uri, **request):
    if request["method"] == "GET":
        ...
        image_position_patient = [
            float(x) for x in tags["ImagePositionPatient"].split("\\")
        ]
                ...
                try:
                    ...
                    if "ORIGINAL" in tags["ImageType"]:
                        ...
                        encoded_images.append(
                            {"name": instance, "label": tags["UserContentLabel"]}
                        )

                        orientation = np.array(
                            [
                                float(x)
                                for x in tags["ImageOrientationPatient"].split("\\")
                            ]
                        )
                        position = np.array(
                            [float(x) for x in tags["ImagePositionPatient"].split("\\")]
                    ...
                    if "DERIVED" in tags["ImageType"]:
                        stackedImages[tags["UserContentLabel"]] = {
                            "name": instance,
